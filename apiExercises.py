...
def getCaloriesBurnedByExercise(query, gender = None, weight_kg = None, height_cm = None, age = None):
    r = getExerciseDataFromQuery(query, gender = gender, weight_kg = weight_kg, height_cm = height_cm, age = age)
    retv = {exercise : r.json()["exercises"][index]["nf_calories"] for index, exercise in enumerate(getExerciseName_s(query, gender = gender, weight_kg = weight_kg, height_cm = height_cm, age = age))}
    return retv
